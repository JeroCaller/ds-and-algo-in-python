...
class Tree():
    ...
    def __init__(
        ...
        Parameters
        ----------
        default_root : bool, default False
            루트 노드가 별로 쓰이지 않을 것 같고 아무 값이나
            할당해도 될 때 'root'라는 문자열을 가지는 노드를 생성하여 root 노드로 지정함.
        delimiter : str, default '.'
            트리 계층 구조를 구분지어주는 기호. 기본은 '.'으로 설정됨. 
            예) a.b.c -> b는 a의 자식 노드, c는 b의 자식 노드.
        always_raise_error : bool, default False
            append(), replace() 등 몇몇 메서드에는 특정 예외 발생 시 
            ...
            for iter_depth in range(depth+1):
                if iter_depth == depth:
                    ...
                    try:
                        ...
                        if not branch_stack:
                            line += sub_dir_line
                        elif iter_depth in dict(branch_stack):
                            line += sub_and_extension
                    else:
                        if iter_depth+1 in dict(branch_stack):
                            line += sub_and_extension
                        else:
                            line += sub_dir_line
                    line += whitespace
                    ...
                    if iter_depth+1 in dict(branch_stack):
                        line += extension + (whitespace*(one_tab_length-1))
                    else:
                        line += whitespace * one_tab_length
    ...
    def search(self, target_node: Node) -> (Node | None):
        ...
        """
        if len(self._adj_list) == 0: return None
        if len(self._adj_list) == 1:
            if self._adj_list[self._root] == []:
                if self._root == target_node:
                    return self._root
                return None

        self._r_adj_list = self._reverseAdjList()
        for node in self._r_adj_list:
            while True:
                if node == target_node:
                    return node
                try:
                    node = self._r_adj_list[node]
                except KeyError:
                    break
        return
    ...
    def append(
        ...
        ------
        ParentNoneError
            트리에 이미 root 노드가 있는 상태에서 
            parent 인자에 None을 입력한 경우.
        NodeNotFoundError
            지정한 부모 노드가 트리 내에 없을 경우.
        NodeAlreadyExistsError
            새로 삽입할 새 노드가 트리 내 기존 노드들 중 하나와 
            그 이름이 겹칠 경우. 

        See Also
        --------
        appendAll : 여러 개의 노드들을 한꺼번에 트리에 삽입해주는 메서드. 
    ...
    def appendAll(self, structure: list[str], raise_error: bool = False):
        ...
        ----------
        structure : list[str]
            부모-자식 관계가 구분 기호(.)를 통해 표현된 문자열의 리스트.
        raise_error : bool, default False
            발생할 수 있는 각종 에러에 대해, raise_error 인자를 True로 설정 시,
        ...
        See Also
        --------
        append : 노드 한 개씩 트리에 추가하는 방식의 메서드.
        ...
        if self._root is None:
            ...
            if r is None:
                if self.always_raise_error or raise_error:
                    ...
                    return
            insert_into_tree(adj_list, r)
        else:
            top_nodes = find_top_nodes(r_adj_list)
            insert_into_tree(adj_list, top_nodes)
    ...
    def remove(
        ...
        target_node : str
            삭제하고자 하는 노드의 문자열 이름.
        mode : REMOVEALL | REMOVEONE | DONTREMOVE, default REMOVEONE
            만약 삭제하고자 하는 노드에 자식 노드가 1개 이상 연결되어 있는 경우, 
            mode 매개변수에 REMOVEALL, REMOVEONE, DONTREMOVE 상수들 중 하나를 
            대입하여 처리한다.
            REMOVEALL
                삭제하고자 하는 노드와 함께 그 자식 노드들도 모두 삭제한다. 
            REMOVEONE
                삭제하고자 하는 노드만 삭제하고, 그 하위 트리들은 모두 
                삭제한 노드의 부모 노드에 종속시킨다. 
            DONTREMOVE 
                만약 삭제하고자 하는 노드가 자식 노드를 가지면 해당 노드와 
    ...
    def replace(
        ...
        ----------
        old_node : str
            바꾸고자 하는 트리 내 기존 노드의 문자열 이름. 
        new_node : str
            old_node의 새 이름.
        raise_error : bool, default False
            해당 메서드 실행 중 발생할 수 있는 에러들에 대해 
            예외를 일으킬지 무시할지에 대한 변수. 
            True 시 각 에러 종류에 따른 예외를 발생. 
        ...
        ------
        NodeNotFoundError
            old_node가 트리 내에 존재하지 않을 경우. 
        NodeAlreadyExistsError
            새로 바꾸고자 하는 new_node가 이미 트리 내에 존재할 경우.
...
class PathTree(Tree):
    ...
    def isAbsPath(self, node: Node | AbsPath) -> (bool):
        ...
        동일해야함. 
        """
        return bool(self._delimiter in node)
    ...
    def splitAbsPath(self, node_abs: AbsPath) -> (tuple[AbsPath, Node]):
        """주어진 노드 절대경로에서 현재 노드 이름과 해당 노드의 부모 노드의 
        절대경로로 분리하여 반환. 
        
        해당 절대경로가 트리에 존재하는지의 여부에 상관없이 작동한다. 
        단, 계층 구분 기호는 PathTree() 생성자의 인자 delimiter와 
        동일해야함. 
    ...
    def search(self, target_node: Node | AbsPath) -> (AbsPath | list[AbsPath] | None):
        ...
        target_node : Node | AbsPath
            노드 이름으로 입력 시 해당 노드 이름을 절대경로로 가지는 모든 
        ...
        if self.isAbsPath(target_node):
            if target_node in self._adj_list:
                return target_node
            else:
                return None
        else:
            results = []
            for key in self._adj_list:
                if self.splitAbsPath(key)[1] == target_node:
                    results.append(key)
            if not results:
                return None
    ...
    def append(
        ...
        ----------
        new_node : Node
            삽입하고자 하는 새 노드 이름을 대입한다. 
        parent : Node | AbsPath | None, default None
            new_node를 연결시킬 부모 노드 이름 또는 부모 노드의 
        ...
        ------
        ParentNoneError
            트리에 이미 root 노드가 있는 상태에서 parent 인자에 None을 입력한 경우. 
        NodeNotFoundError
            지정한 부모 노드가 트리 내에 없을 경우.
        PathAlreadyExistsError
            새로 삽입하고자 하는 새 노드의 절대경로가 이미 트리 내에 있을 경우. 

        Examples
        --------
        'parent' 매개변수에 노드 이름으로 대입 시 트리 내 같은 이름을 가지는 
        모든 노드에 새 노드들을 삽입한다.
        ...
        반면, 절대경로로 입력 시, 그 절대경로에만 새 노드를 삽입한다.
        ...
        for p in parents:
            ...
            if self.search(new_node_abs):
                if self.always_raise_error or raise_error:
                    ...
                    return
            try:
                self._adj_list[p].append(new_node)
            except KeyError:
                self._adj_list[p] = [new_node]
            else:
                self._adj_list[p].sort()
            self._adj_list[new_node_abs] = []
            self._node_number += 1
    ...
    def appendAbs(self, new_path: AbsPath, raise_error: bool = False) -> (None):
        ...
        PathAlreadyExistsError
            새로 삽입하고자 하는 새 노드의 절대경로가 이미 트리 내에 있을 경우.
        RootNotUniqueError
            새 노드의 절대경로에 적힌 루트 노드의 이름과 트리에 이미 존재하는 
            루트 노드 이름과 다를 경우. 
    ...
    def appendAll(self, structure: list[str], raise_error: bool = False) -> (None):
        ...
        ----------
        structure : list[str]
            부모-자식 관계가 구분 기호(.)를 통해 표현된 leaf 노드들의 문자열 리스트.
        raise_error : bool, default False
            발생할 수 있는 각종 에러에 대해, raise_error 인자를 True로 설정 시,
            에러의 종류에 따라 예외를 발생시킨다. 예외 종류에 대한 정보는 
            append() 메서드의 독스트링 참조. 
        ...
        ------
        RootNotUniqueError() 
            appendAll() 메서드에만 존재하는 예외
            structure 인자 속 정보에 root 노드가 두 개 이상 존재할 경우. 
        ...
        리스트 안에 입력되는 모든 정보들은 트리 내 모든 leaf 노드들의 
        절대경로들로 구성해야함.
    ...
    def remove(
        ...
        mode : REMOVEALL | REMOVEONE | DONTREMOVE, default REMOVEONE
            ...
            REMOVEONE
                ...
                이름이 겹치면 하나로 합친다(Examples 참고). 해당 모드에서는 root 노드를 삭제할 수 없다.
            DONTREMOVE
                만약 삭제하고자 하는 노드가 자식 노드를 가지면 해당 노드와 
                그 자식 노드 모두 삭제하지 않는다. 오로지 자식 노드가 없는 leaf 노드일 
                경우에만 삭제한다. 
        ...
        `mode` 매개변수에 REMOVEONE를 대입할 경우, 삭제된 노드의 하위 노드를 
        삭제된 노드의 부모 노드에 종속시킬 때, 해당 부모 노드의 자식 노드와 그 
        이름이 겹치면 하나로 합친다.
        ...
        if mode == REMOVEALL:
            ...
            while not queue.isEmpty():
                ...
                p = self.getParent(node)
                try:
                    self._adj_list[p].remove(self.basename(node))
                except KeyError:
                    pass
                try:
                    del self._adj_list[node]
            ...
            if self.isAbsPath(target_node):
                ...
                for v in t_nodes_abs:
                    heapq.heappush(temp_list, (-len(v), v))
                t_nodes_abs.clear()
                while temp_list:
    ...
    def replace(
        ...
        ----------
        old_node : Node | AbsPath
            바꾸고자 하는 트리 내 기존 노드의 이름 또는 절대경로. 
        ...
        NodeNotFoundError
            old_node가 트리 내에 존재하지 않을 경우. 
        PathAlreadyExistsError
            새로 바꿀 노드의 절대경로와 똑같은 경로를 가지는 노드가 
            이미 트리 내에 존재할 경우. 
        ...
        if old_nodes_abs is None:
            if self.always_raise_error or raise_error:
                ...
                return False
        if isinstance(old_nodes_abs, str):
            old_nodes_abs = [old_nodes_abs]
    ...
    def getAllLeafAbs(
        ...
        Parameters
        ----------
        how_to_sort: ALPHABET | LENGTH, default ALPHABET
            leaf 노드들의 절대경로들을 리스트에 저장하고 반환할 때 
            리스트의 정렬방식. 
